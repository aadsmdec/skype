...
window.logEachRequestTime = false;
window.logTotalRequestsTime = true;
window.totalRequestsTime = 0;
...
bootstrap("require/browser", function (require) {
        ...
        try {
            ...
            request.onreadystatechange = function () {
                if (request.readyState === 4) {
                    ...
                    if (window.logEachRequestTime) {
                        console.log(url + " " + (endTime - startTime));
                    }
                    if (window.logTotalRequestsTime) {
                        window.totalRequestsTime += (endTime - startTime);
                    }
                    onload();
                }
            };
    ...
    WebsocketConnection.prototype.request = function(url) {
        ...
        return this._connection.then(function(connection) {
            ...
            deferred.startTime = window.performance.now();
    ...
    WebsocketConnection.prototype._onmessage = function(event) {
        ...
        if (response) {
            ...
            var endTime = window.performance.now();
            if (window.logEachRequestTime = true) {
                console.log(data.length + " " + (endTime - deferred.startTime));
            }
    ...
    if (window.useMrWebsocket) {
        ...
        Require.read = function (url) {
            ...
            var startTime = window.performance.now();
            return websocketConnection[websocketIx].request(url)
            .then(function(response) {
                /*
                var endTime = window.performance.now();
                if (window.logEachRequestTime) {
                    console.log(response.length + " " + (endTime - startTime));
                }
                if (window.logTotalRequestsTime) {
                    window.totalRequestsTime += (endTime - startTime);
                }
